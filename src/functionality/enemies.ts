...
import {actionChoice, errorMessages, floatFromString, numFromString} from "./data";
...
export class enemy {
  ...
  private noCounterWeapons: Set<string> = new Set();
  private noCounterSpells: Set<string> = new Set();
  ...
  turnStart(): void {
    ...
    if (this.health > this.maxHealth) {
      this.health = Math.max(this.maxHealth, this.health - AI_VALUES.ENEMY_OVERHEAL_DECAY);
    }
    this.modifyMana(this.turnManaRegen);
    if (this.mana > this.maxMana) {
      this.mana = Math.max(this.maxMana, this.mana - AI_VALUES.ENEMY_MANA_DECAY);
    }
    if (this.poison > 0) {
      this.poison--;
    }
  ...
  constructor(blueprint: string | enemy = "EMPTY") {
    if (typeof blueprint == "string") {
      ...
      if (this.real) {
        ...
        for (let weaponry of blueprint.weapons) {
          this.weapons.push(new weapon(weaponry));
        }
        for (let magic of blueprint.spells) {
          this.spells.push(new spell(magic));
        }
        this.initialSpell = blueprint.initialSpell;
        this.deathSpell = blueprint.deathSpell ? new spell(blueprint.deathSpell) : undefined;
        ...
        for (let weaponry of blueprint.noCounterWeapons) {
          this.noCounterWeapons.add(weaponry);
        }
        for (let magic of blueprint.noCounterSpells) {
          this.noCounterSpells.add(magic);
        }
  ...
  loadFromFile(blueprint: string = "EMPTY"): void {
    ...
    this.noCounterWeapons.clear();
    this.noCounterSpells.clear();
    if (blueprint == "EMPTY") {
      return;
    }
    try {
  ...
  chooseAction(
    ...
    let slot: number | undefined = 0; //For holding slot selection
      ...
      case 3:
        switch (timing) {
          ...
          case 1: //Responding to weapon attack
            ...
            if (this.checkCounter(false, itemName1) || (timing == 4 && this.checkCounter(false, itemName2))) {
  ...
  check(
    ...
    const currentConstRegen = this.turnRegen;
    const currentTurnManaRegen = this.turnManaRegen;
    const currentMaxMana = this.maxMana;
    const currentMaxHealth = this.maxHealth;
    const currentProjectiles = this.projectiles;
    const currentBleed = this.bleed;
    const currentPoison = this.poison;
    const currentTempRegen = this.tempRegen;
    switch (type) {
      case false: //Weapon
        //Check weapon exists
        if (slot >= this.weapons.length || slot < 0 || !this.weapons[slot].getReal()) {
          return false;
        }
        switch (timing) {
          case 0:
        ...
        if (
          ...
          (this.health + this.weapons[slot].getHealthChange() < 0 ||
            (!kamikaze && this.health + this.weapons[slot].getHealthChange() == 0))
        ) {
          return false;
        }
        //Mana cost
        if (this.weapons[slot].getManaChange() < 0 && this.mana + this.weapons[slot].getManaChange() < 0) {
          return false;
        }
        //Projectile cost
        if (
        ...
        this.modifyProjectiles(this.weapons[slot].getProjectileChange());
        if (this.health <= 0) {
          //Would die
      ...
      case true: //Spell
        //Check spell exists
        if (slot >= this.spells.length || slot < 0 || !this.spells[slot].getReal()) {
          return false;
        }
        //Check cooldown
        ...
        if (
          ...
          (this.health + this.spells[slot].getHealthChange() < 0 ||
            (!kamikaze && this.health + this.spells[slot].getHealthChange() == 0))
        ) {
          return false;
        }
        if (this.spells[slot].getManaChange() < 0 && this.mana + this.spells[slot].getManaChange() < 0) {
          return false;
        }
        if (
          this.spells[slot].getProjectileChange() < 0 &&
          this.projectiles + this.spells[slot].getProjectileChange() < 0
        ) {
          return false;
        }
        //Kamikaze check
        ...
        this.modifyTurnManaRegen(this.spells[slot].getTurnManaRegenModifier());
   ...
   */
  chooseSpell(type: number, timing: 0 | 1 | 2 | 3 = 0, kamikaze: boolean = false, firstTurn: boolean = false): number {
    if (this.spells.length == 0) {
      return -1;
    }
    /**Holds slot numbers of spells which could be chosen */
    let possibleSpells: number[] = [];
    for (let [i, magic] of this.spells.entries()) {
      if (magic.checkSpellType(type) && this.check(true, i, timing, kamikaze, firstTurn)) {
   ...
   */
  chooseWeapon(timing: 0 | 1 | 2 | 3 = 0, kamikaze: boolean = false, firstTurn: boolean = false): actionChoice {
    if (this.weapons.length == 0) {
      return {actionType: 0};
    }
    let possibleWeapons: number[] = [];
    for (let [i, weaponry] of this.weapons.entries()) {
      if (this.check(false, i, timing, kamikaze, firstTurn)) {
        possibleWeapons.push(i);
        if (!weaponry.getDualWield()) {
          possibleWeapons.push(i);
        }
      }
    }
    ...
    let selection: number = possibleWeapons[randomInt(0, possibleWeapons.length)];
    if (this.weapons[selection].getDualWield()) {
      possibleWeapons.length = 0;
      for (let i = 0; i < this.weapons.length; i++) {
        if (this.checkDualWield(selection, i, timing, kamikaze, firstTurn)) {
          possibleWeapons.push(i);
        }
      }
      if (possibleWeapons.length == 0) {
      ...
      return {actionType: 3, slot1: selection, slot2: possibleWeapons[randomInt(0, possibleWeapons.length)]};
    }
    return {actionType: 1, slot1: selection};
  }
  ...
  chooseWeaponCounterSpell(firstTurn: boolean = false): number {
    ...
    let possibleSpells: number[] = [];
    for (let [i, magic] of this.spells.entries()) {
      if ((magic.getCounterSpell() == 2 || magic.getCounterSpell() == 3) && this.check(true, i, 1, false, firstTurn)) {
        possibleSpells.push(i);
      }
    }
    if (possibleSpells.length == 0) {
  ...
  chooseSpellCounterSpell(firstTurn: boolean = false): number {
    ...
    let possibleSpells: number[] = [];
    for (let [i, magic] of this.spells.entries()) {
      if ((magic.getCounterSpell() == 1 || magic.getCounterSpell() == 3) && this.check(true, i, 2, false, firstTurn)) {
        possibleSpells.push(i);
      }
    }
    if (possibleSpells.length == 0) {
   ...
   * @returns an object {actionType, slot1?, slot2?} actionType 0 is nothing, 1 is a weapon, 2 is a spell, 3 is two weapons
   */
  chooseAttack(timing: 0 | 1 | 2 | 3 = 0, kamikaze: boolean = false, firstTurn: boolean = false): actionChoice {
    ...
    for (let [i, magic] of this.spells.entries()) {
      if (magic.checkSpellType(1) && this.check(true, i, timing, kamikaze, firstTurn)) {
        possibleAttacks.push(-(i + 1));
        possibleAttacks.push(-(i + 1));
      }
    }
    ...
    let slot: number = possibleAttacks[randomInt(0, possibleAttacks.length)];
    //Weapon
    if (slot > 0) {
      slot--;
      if (this.weapons[slot].getDualWield()) {
        ...
        for (let i = 0; i < this.weapons.length; i++) {
          if (this.checkDualWield(slot, i, timing, kamikaze, firstTurn)) {
        ...
        return {actionType: 3, slot1: slot, slot2: possibleAttacks[randomInt(0, possibleAttacks.length)]};
      }
      return {actionType: 1, slot1: slot};
    } else {
  ...
  checkDualWield(
    ...
    if (this.currentBonusActions <= 0 || (timing != 0 && this.currentBonusActions <= 1)) {
      return false;
    }
    const currentHealth: number = this.health;
    const currentProjectiles: number = this.projectiles;
    const currentMana: number = this.mana;
    const currentPoison: number = this.poison;
    const currentBleed: number = this.bleed;
    if (weapon1 == weapon2) {
      return false;
    }
    if (!this.weapons[weapon2].getDualWield() || !this.weapons[weapon1].getDualWield()) {
      return false;
    }
    if (weapon2 >= this.weapons.length || !this.weapons[weapon2].getReal()) {
      return false;
    }
    ...
    if (
      ...
      (this.health + this.weapons[weapon2].getHealthChange() < 0 ||
        (!kamikaze && this.health + this.weapons[weapon2].getHealthChange() == 0))
    ) {
      this.health = currentHealth;
      this.mana = currentMana;
      this.projectiles = currentProjectiles;
    ...
    if (this.weapons[weapon2].getManaChange() < 0 && this.mana + this.weapons[weapon2].getManaChange() < 0) {
      this.health = currentHealth;
      this.mana = currentMana;
      this.projectiles = currentProjectiles;
      return false;
  ...
  addNoCounter(type: boolean, itemName: string): void {
    if (type) {
      this.noCounterSpells.add(itemName);
    } else {
      this.noCounterWeapons.add(itemName);
    }
    return;
  }
  ...
  checkCounter(type: boolean, itemName: string): boolean {
    if (type) {
      return this.noCounterSpells.has(itemName);
    } else {
      return this.noCounterWeapons.has(itemName);
    }
  }
