 ...
 */
export function weaponDeclare(attacker: player | enemy, weapon1: weapon, weapon2?: weapon): void {
...
export function WeaponAttack({
  ...
  if (weapon1.getPropSelfDamage() > 0) {
    ...
    if (weapon2?.getPropSelfDamage() > 0) {
      ...
      battleLog.push((outputText = `${-Math.round(100 * damageBuffer)}% health`));
      attackerEffects.push(
      ...
      battleLog.push(
        (outputText = `${-Math.round(100 * weapon1.getPropSelfDamage())}% health, then ${-Math.round(
      ...
      attacker.propDamage(weapon1.getPropSelfDamage());
      battleLog.push((outputText = `${-Math.round(100 * weapon1.getPropSelfDamage())}% health`));
      attackerEffects.push(
        <div className="ion-text-center" key="propSelfDamage">
          {outputText}
        </div>
    ...
    if (weapon2?.getPropSelfDamage() > 0) {
      ...
      attacker.propDamage(weapon1.getPropSelfDamage());
      battleLog.push(
        (outputText = `${-Math.round(100 * weapon2!.getPropSelfDamage())}% health, then ${-Math.round(
      ...
      attacker.propDamage(damageBuffer);
      battleLog.push((outputText = `${-Math.round(100 * damageBuffer)}% of health recovered`));
      attackerEffects.push(
        <div className="ion-text-center" key="propSelfDamage">
          {outputText}
        </div>
      ...
      attacker.propDamage(weapon1.getPropSelfDamage());
      battleLog.push((outputText = `${-Math.round(100 * weapon1.getPropSelfDamage())}% of health recovered`));
      attackerEffects.push(
        <div className="ion-text-center" key="propSelfDamage">
          {outputText}
        </div>
  ...
  selfDamage: {
    if (weapon1.getEffectType()[1] == 1 || weapon1.getEffectType()[1] == 2) {
      if (weapon2?.getEffectType()[1] == 1 || weapon2?.getEffectType()[1] == 2) {
    ...
    } else if (weapon2?.getEffectType()[1] == 1 || weapon2?.getEffectType()[1] == 2) {
      damageBuffer = attacker.flatDamage(
        weapon2.getFlatSelfDamage(),
        weapon2.getFlatSelfMagicDamage(),
        weapon2.getFlatSelfArmourPiercingDamage(),
  ...
  {
    let selfPoison: number = weapon1.getSelfPoison() + (weapon2?.getSelfPoison() ?? 0);
    let selfBleed: number = weapon1.getSelfBleed() + (weapon2?.getSelfBleed() ?? 0);
  ...
  if (
    weapon1.getEffectType()[1] < 2 &&
    weapon1.getEffectType()[0] < 2 &&
    (weapon2 == undefined || (weapon2.getEffectType()[1] < 2 && weapon2.getEffectType()[0] < 2))
  ) {
    return attackerEffects;
  }
  let targetEffects: React.ReactNode[] = [
    <div className="ion-text-center" key="target-effects">
      Target effects:
    </div>,
  ];
  let hits1: number;
  let hits2: number | undefined;
  if (counter) {
...
export function WeaponHit({ weaponry, attacker, target, battleLog }: WeaponHitProps): React.ReactNode {
  ...
  if (weaponry.getPropDamage() > 0) {
    battleLog.push((outputText = `${-Math.round(100 * weaponry.getPropDamage())}% health`));
    hitEffects.push(
    ...
    battleLog.push((outputText = `${-Math.round(100 * weaponry.getPropDamage())} % of health recovered`));
    hitEffects.push(
      <div className="ion-text-center" key="propDamage">
        {outputText}
      </div>
  ...
  if (weaponry.getEffectType()[1] > 1) {
    ...
    let healthLoss: number = target.flatDamage(p, m, a, weaponry.getTargetOverHeal());
    if (healthLoss > 0) {
...
export function SpellCast({ magic, caster, target, timing, battleLog }: SpellCastProps): React.ReactNode {
  ...
  if (caster != undefined) {
    ...
    if (magic.getPropSelfDamage() > 0) {
      battleLog.push((outputText = `${-Math.round(100 * magic.getPropSelfDamage())}% health`));
      attackerEffects.push(
        <div className="ion-text-center" key="propSelfDamage">
          {outputText}
        </div>
      ...
      battleLog.push((outputText = `${-Math.round(100 * magic.getPropSelfDamage())}% of health recovered`));
      attackerEffects.push(
        <div className="ion-text-center" key="propSelfDamage">
          {outputText}
        </div>
    ...
    if (magic.getEffectType()[0] == 1 || magic.getEffectType()[0] == 2) {
      if (magic.getSelfPoison() != 0 || magic.getSelfBleed() != 0 || magic.getTempRegenSelf() != 0) {
          ...
          if (caster.modifyPoison(magic.getSelfPoison())) {
            outputText +=
              magic.getSelfPoison() == -255
                ? "Poison cured, "
                : `${magic.getSelfPoison() > 0 ? "+" : ""}${magic.getSelfPoison()} poison, `;
          } else {
            outputText += "Poison resisted, ";
          }
        }
          ...
          if (caster.modifyBleed(magic.getSelfBleed())) {
            outputText +=
              magic.getSelfBleed() == -255
                ? "Bleed cured, "
                : `${magic.getSelfBleed() > 0 ? "+" : ""}${magic.getSelfBleed()} bleed, `;
          } else {
            outputText += "Bleed resisted, ";
          }
        }
        if (magic.getTempRegenSelf() != 0) {
          ...
          outputText += `${magic.getTempRegenSelf() > 0 ? "+" : ""}${magic.getTempRegenSelf()} regeneration, `;
        }
        battleLog.push((outputText = outputText.slice(0, -2)));
        attackerEffects.push(
          <div className="ion-text-center" key="dotSelf">
      ...
      if (magic.getPoisonResistModifier() != 0 || magic.getBleedResistModifier() != 0) {
        ...
        if (magic.getBleedResistModifier() != 0) {
          ...
          outputText += `${magic.getBleedResistModifier() > 0 ? "+" : ""}${Math.round(
      ...
      if (
        ...
        (caster instanceof player && magic.getBattleRegenModifier() != 0)
      ) {
        outputText = "";
        if (magic.getMaxHealthModifier() != 0) {
          caster.modifyMaxHealth(magic.getMaxHealthModifier());
          outputText += `${magic.getMaxHealthModifier() > 0 ? "+" : ""}${magic.getMaxHealthModifier()} max health, `;
        }
        ...
        if (caster instanceof player && magic.getBattleRegenModifier() != 0) {
      ...
      if (
        ...
        (caster instanceof player && magic.getBattleManaRegenModifier() != 0)
      ) {
        outputText = "";
        if (magic.getMaxManaModifier() != 0) {
          caster.modifyMaxMana(magic.getMaxManaModifier());
          outputText += `${magic.getMaxManaModifier() > 0 ? "+" : ""}${magic.getMaxManaModifier()} maximum mana, `;
        }
        ...
        if (caster instanceof player && magic.getBattleManaRegenModifier() != 0) {
      ...
      if (magic.getFlatArmourModifier() != 0 || magic.getFlatMagicArmourModifier() != 0) {
        ...
        battleLog.push((outputText = outputText.slice(0, -2)));
        attackerEffects.push(
          <div className="ion-text-center" key="flatSelfArmourModifiers">
            {outputText}
          </div>
        );
      }
      if (magic.getPropArmourModifier() != 0 || magic.getPropMagicArmourModifier() != 0) {
        ...
        if (magic.getPropArmourModifier() != 0) {
          ...
          outputText += `${magic.getPropArmourModifier() > 0 ? "+" : ""}${Math.round(
        ...
        if (magic.getPropMagicArmourModifier() != 0) {
          ...
          outputText += `${magic.getPropMagicArmourModifier() > 0 ? "+" : ""}${Math.round(
            100 * magic.getPropMagicArmourModifier()
          )}% magic damage received, `;
        }
        battleLog.push((outputText = outputText.slice(0, -2)));
        attackerEffects.push(
          <div className="ion-text-center" key="propSelfArmourModifiers">
            {outputText}
          </div>
        );
      }
      if (
        ...
        battleLog.push((outputText = outputText.slice(0, -2)));
        attackerEffects.push(
          <div className="ion-text-center" key="flatSelfDamageModifiers">
            {outputText}
          </div>
        );
      }
      if (
        ...
        if (magic.getPropDamageModifier() != 0) {
          ...
          outputText += `${magic.getPropDamageModifier() > 0 ? "+" : ""}${Math.round(
        ...
        if (magic.getPropMagicDamageModifier() != 0) {
          ...
          outputText += `${magic.getPropMagicDamageModifier() > 0 ? "+" : ""}${Math.round(
        ...
        if (magic.getPropArmourPiercingDamageModifier() != 0) {
          ...
          outputText += `${magic.getPropArmourPiercingDamageModifier() > 0 ? "+" : ""}${Math.round(
            100 * magic.getPropArmourPiercingDamageModifier()
          )}% armour piercing damage dealt, `;
        }
        battleLog.push((outputText = outputText.slice(0, -2)));
        attackerEffects.push(
          <div className="ion-text-center" key="propSelfDamageModifiers">
            {outputText}
          </div>
        );
      }
    ...
    if (magic.getEvadeChanceModifier() != 0) {
      ...
      battleLog.push(
        (outputText = `${magic.getEvadeChanceModifier() > 0 ? "+" : ""}${Math.round(
    ...
    if (magic.getCounterAttackChanceModifier() != 0) {
      ...
      battleLog.push(
        (outputText = `${magic.getCounterAttackChanceModifier() > 0 ? "+" : ""}${Math.round(
          100 * magic.getCounterAttackChanceModifier()
        )}% counter attack chance`)
      );
      attackerEffects.push(
        <div className="ion-text-center" key="selfCounterAttackChanceModifier">
          {outputText}
        </div>
      );
    }
...
function SpellHit({ magic, caster, target, battleLog }: SpellHitProps): React.ReactNode {
  ...
  if (magic.getPoison() != 0 || magic.getBleed() != 0 || magic.getTempRegen() != 0) {
    ...
    if (magic.getPoison() != 0) {
      if (target.modifyPoison(magic.getPoison())) {
        outputText += `${magic.getPoison() > 0 ? "+" : ""}${magic.getPoison()} poison, `;
      } else {
        outputText += "Poison resisted, ";
      }
    }
    if (magic.getBleed() != 0) {
      if (target.modifyBleed(magic.getBleed())) {
        outputText += `${magic.getBleed() > 0 ? "+" : ""}${magic.getBleed()} bleed, `;
      } else {
        outputText += "Bleed resisted, ";
      }
    }
  ...
  if (
    ...
    if (target instanceof player && magic.getBattleRegenModifierEnemy() != 0) {
  ...
  if (
    ...
    (target instanceof player && magic.getBattleManaRegenModifierEnemy() != 0)
  ) {
    outputText = "";
    ...
    if (magic.getManaChangeEnemy() != 0) {
      ...
      outputText += `${magic.getManaChangeEnemy() > 0 ? "+" : ""}${magic.getManaChangeEnemy()} mana, `;
    }
    ...
    if (target instanceof player && magic.getBattleManaRegenModifierEnemy() != 0) {
  ...
  if (magic.getPoisonResistModifierEnemy() != 0 || magic.getBleedResistModifierEnemy() != 0) {
    ...
    if (magic.getPoisonResistModifierEnemy() != 0) {
      ...
      outputText += `${magic.getPoisonResistModifierEnemy() > 0 ? "+" : ""}${Math.round(
    ...
    if (magic.getBleedResistModifierEnemy() != 0) {
      ...
      outputText += `${magic.getBleedResistModifierEnemy() > 0 ? "+" : ""}${Math.round(
  ...
  if (magic.getPropArmourModifierEnemy() != 0 || magic.getPropMagicArmourModifierEnemy() != 0) {
    ...
    if (magic.getPropArmourModifierEnemy() != 0) {
      ...
      outputText += `${magic.getPropArmourModifierEnemy() > 0 ? "+" : ""}${Math.round(
    ...
    if (magic.getPropMagicArmourModifierEnemy() != 0) {
      ...
      outputText += `${magic.getPropMagicArmourModifierEnemy() > 0 ? "+" : ""}${Math.round(
  ...
  if (
    ...
    if (magic.getPropDamageModifierEnemy() != 0) {
      ...
      outputText += `${magic.getPropDamageModifierEnemy() > 0 ? "+" : ""}${Math.round(
    ...
    if (magic.getPropMagicDamageModifierEnemy() != 0) {
      ...
      outputText += `${magic.getPropMagicDamageModifierEnemy() > 0 ? "+" : ""}${Math.round(
    ...
    if (magic.getPropArmourPiercingDamageModifierEnemy() != 0) {
      ...
      outputText += `${magic.getPropArmourPiercingDamageModifierEnemy() > 0 ? "+" : ""}${Math.round(
